...
execute if entity @s[scores={shearActionsRemaining=0}] as @s run tellraw @p[nbt={SelectedItem:{components:{"minecraft:enchantments":{levels:{"enchantology:butchery":2}}}, count: 1, id: "minecraft:shears"}}] {"text":"You've already fractured its bones...", "color":"gray", "italic":true}
execute if entity @s[scores={shearActionsRemaining=0}] as @s run tellraw @p[nbt={SelectedItem:{components:{"minecraft:enchantments":{levels:{"enchantology:butchery":3}}}, count: 1, id: "minecraft:shears"}}] {"text":"You've already fractured its bones...", "color":"gray", "italic":true}
...
# Special: KILL SKELETON if all rotten flesh has been sliced off
