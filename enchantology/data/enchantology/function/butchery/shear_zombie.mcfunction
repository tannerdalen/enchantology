...
execute as @s[scores={shearActionsRemaining=0}] if items entity @s weapon.mainhand *[minecraft:enchantments~[{enchantments:"enchantology:butchery",levels:2}]] run tellraw @s {"text":"You've already gnashed the flesh off its bones...", "color":"gray", "italic":true}
execute as @s[scores={shearActionsRemaining=0}] if items entity @s weapon.mainhand *[minecraft:enchantments~[{enchantments:"enchantology:butchery",levels:3}]] run tellraw @s {"text":"You've already gnashed the flesh off its bones...", "color":"gray", "italic":true}
